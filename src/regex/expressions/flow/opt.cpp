...
// core implementation
ParserOpt::ParserOpt(utils::ParserPtr ptr)
  : ptr_(std::move(ptr)) {}

bool ParserOpt::match(stream::Stream & is, DataContext & context) const {
  ptr_->match(is, context);
  return true;
}
...
// interface
Regex opt(Regex r) {
  return new ParserOpt(std::move(r).ptr());
}
