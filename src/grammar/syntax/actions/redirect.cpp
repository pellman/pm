...
public:
  ...
  // Name() deleted
  // CONSTRUCTORS
  Redirect(const std::string & n);
  Redirect(std::string && n);
  // PRESERVING METHODS
...
// core implementation
Redirect::Redirect(const std::string & n)
  : n_(n) {}

Redirect::Redirect(std::string && n)
  : n_(std::move(n)) {}

grammar::Action Redirect::interpret(grammar::Grammar & g, InterpretInfo & ii) const {
  size_t index;
...
// interface
Action ac(const std::string & n) {return new Redirect(n);}
Action ac(std::string && n) {return new Redirect(std::move(n));}
