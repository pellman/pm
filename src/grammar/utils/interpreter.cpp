...
size_t Interpreter::add_name(const std::string & name, Model & model, InterpretInfo & ii) const {
  auto it = ii.action_map.find(name);
  if(it == ii.action_map.end()) {
    return ii.action_map[name] = model.add(afalse());
  }
  else {
    return it->second;
  }
