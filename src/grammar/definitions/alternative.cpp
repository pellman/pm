...
// core
class InterpreterAlternative
  : public utils::Interpreter {
public:
  InterpreterAlternative(std::vector<utils::InterpreterPtr> ptrseq);
...
// core implementation
InterpreterAlternative::InterpreterAlternative(std::vector<utils::InterpreterPtr> ptrseq)
  : ptrseq_(std::move(ptrseq)) {}

utils::Action InterpreterAlternative::interpret(utils::Model & model, utils::InterpretInfo & ii) const {
  ...
  return utils::alt(std::move(res_actions));
}

} // namespace

// interface
Definition alt(std::vector<Definition> dseq) {
  std::vector<utils::InterpreterPtr> ptrseq;
  ptrseq.reserve(dseq.size());
  for(auto & d : dseq) {
    ptrseq.push_back(std::move(d).ptr());
  }
  return new InterpreterAlternative(std::move(ptrseq));
}
