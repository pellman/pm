 ...
 */
#ifndef __PM_REGEXP_EXPRESSIONS_TYPICAL_HPP
#define __PM_REGEXP_EXPRESSIONS_TYPICAL_HPP
...
// common language notions
// = single symbols =
Regexp c_id_head_letter();
Regexp c_id_tail_letter();
Regexp digit_letter();
template<typename Int>
Regexp digit() {return concat(digit_letter(), cchar_to_digit<Int>());}
Regexp latin_letter();
...
Regexp c_id();
template<typename Int>
Regexp decimal() {return concat(digit<Int>(), hide(concat(swap(), star(concat(digit<Int>(), smul<Int>(10), sadd<Int, Int>())), swap())));}
