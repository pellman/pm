 ...
 */
#ifndef __PM_DATASET_SYNTAX_FUNCTIONS_TYPICAL_HPP
#define __PM_DATASET_SYNTAX_FUNCTIONS_TYPICAL_HPP
...
Function set(const std::string & target, Args && ... args) {return dfun(type_abuse::funs::set<NewStoredValue>(std::forward<Args>(args) ...), target);}
template<typename NewStoredValue, typename ... Args>
Function set(std::string && target, Args && ... args) {return dfun(type_abuse::funs::set<NewStoredValue>(std::forward<Args>(args) ...), std::move(target));}
