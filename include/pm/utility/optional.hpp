 ...
 */
#ifndef PM_UTILITY_OPTIONAL_HPP
#define PM_UTILITY_OPTIONAL_HPP
...
template<typename T>
bool Optional<T>::nonempty() const {
  return has_value_;
}
...
template<typename T>
const T & Optional<T>::val() const {
  return value_;
}

template<typename T>
template<typename ... Args>
void Optional<T>::set_(Args && ... args) {
  new (&value_) T(std::forward<Args>(args) ...);
}

template<typename T>
void Optional<T>::unset_() {
  value_.~T();
}
